...
export const DEFAULT_TEMPLATES: TemplateSeed[] = [
  {
    ...
    systemPrompt: [
      ...
      "2. For each repo you'll touch, read its top-level docs first (CLAUDE.md, AGENTS.md, README.md) and any nearby conventions. Match the project's existing patterns and style — do not introduce new abstractions or libraries unless the brief requires them. CONTEXT.md takes precedence on cross-cutting concerns; per-repo docs win for project-specific patterns; MEMORY.md is your own notes — trust them but verify if anything seems stale.",
